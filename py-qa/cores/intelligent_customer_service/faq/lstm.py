# -*- coding: utf-8 -*-
"""
Classic LSTM Encoder for Online QA.
...
from cores.faq.data_process import data_loader
    ...
    for e in range(int(config.ini["hyparams"]["num_epochs"])):
        if e % 3 == 1:
            ...
            for x, y, lens in test_dl:
                ...
                if acc > acc_flag and acc > 0.8132:
                    acc_flag = acc
                    logger.warn("The best accuracy hits %.4f" % acc)
                    torch.save(model.state_dict(), config.ini["modelDir"] + "/model-%s.pt" % round(acc, 4))
...
if __name__ == '__main__':
    ...
    model = LSTM(embedding=embedding, hidden_size=int(config.ini["nnparams"]["hidden_size"]), num_layers=int(config.ini["nnparams"]["num_layers"]), num_classes=int(config.ini["nnparams"]["num_classes"]))
    model.to(device)
